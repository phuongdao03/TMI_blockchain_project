...
- `STAGING_SSH_KNOWN_HOSTS`, `PRODUCTION_SSH_KNOWN_HOSTS` containing the
  pinned OpenSSH host-key line for each VPS
...
4. The deploy script serializes releases with `flock`, validates Compose and
   the 0600 environment file, waits for container health, then verifies local
   TLS health at `https://APP_DOMAIN/health`.
