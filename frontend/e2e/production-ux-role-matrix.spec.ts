...
const personas = [
  ...
  { name: "moderator", cookie: "reviewer", path: "/dashboard", workspace: true },
  { name: "super admin", cookie: "super-admin", path: "/admin/dashboard", workspace: true },
] as const;

async function authenticate(context: BrowserContext, persona: (typeof personas)[number]) {
  await context.addCookies([
    {
      name: "cns_access",
      value: persona.cookie === "super-admin" ? "e2e-super-admin-access" : "e2e-access",
      domain: "127.0.0.1",
      path: "/",
      httpOnly: true,
      sameSite: "Lax",
...
async function setTheme(page: Page, workspace: boolean, theme: "sáng" | "tối") {
  ...
  await expect(page.locator("html")).toHaveAttribute("data-theme", theme === "sáng" ? "light" : "dark");
}

for (const persona of personas) {
  test(`${persona.name} primary shell stays within mobile/tablet/desktop viewports in both themes`, async ({ context, page }, testInfo) => {
    ...
    if (persona.cookie === "super-admin") {
      await expect(page.locator('section[aria-label="Chỉ số cần theo dõi"]')).toBeVisible();
        ...
        if (width !== 768) {
          ...
          expect(
            audit.violations.map((violation) => ({
              id: violation.id,
              elements: violation.nodes.slice(0, 4).map((node) => ({ target: node.target, data: node.any[0]?.data })),
            })),
