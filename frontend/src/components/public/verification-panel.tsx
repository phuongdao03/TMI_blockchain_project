    ...
    <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
      <section className="rounded-3xl border border-white/10 bg-white/5 p-6 sm:p-8">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-400">
          Đối chiếu hash dữ liệu với bản ghi blockchain mà không hiển thị thông
          tin riêng tư.
        </p>
        {!token && (
          <form
          ...
          >
            <div className="grid grid-cols-2 rounded-xl bg-ink-950 p-1">
              <button
                className={`min-h-10 rounded-lg text-xs font-bold ${mode === "number" ? "bg-white text-ink-950" : "text-slate-400"}`}
              ...
              <button
                className={`min-h-10 rounded-lg text-xs font-bold ${mode === "transaction" ? "bg-white text-ink-950" : "text-slate-400"}`}
            ...
            />
            <button
              className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary-600 text-sm font-bold"
              type="submit"
            >
      ...
      >
        {result.isFetching ? (
          ...
          <div className="grid min-h-72 place-items-center text-center text-red-300">
            Không thể kết nối dịch vụ xác minh.
          </div>
        ) : result.data && presentation && Icon ? (
          <div>
            <Icon
              className={`size-12 ${presentation[1]} ${result.data.status === "PENDING" ? "animate-spin" : ""}`}
            />
            <p
              className={`mt-5 text-xs font-bold uppercase tracking-[0.18em] ${presentation[1]}`}
            >
              Kết quả xác minh
            </p>
            ...
            <dl className="mt-7 grid gap-4 text-sm">
              {[
                ...
                ([label, data]) =>
                  data && (
                    <div
                      className="grid gap-1 border-b border-white/10 pb-3 sm:grid-cols-[8rem_1fr]"
                      key={label}
                    >
