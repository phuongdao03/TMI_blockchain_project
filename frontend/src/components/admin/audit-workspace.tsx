  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <header>
        ...
        <h1 className="mt-2 text-3xl font-bold">Nhật ký audit</h1>
        <p className="mt-2 text-sm text-neutral-600">
          Bản ghi chỉ đọc cho các thao tác quan trọng trong hệ thống.
        </p>
      </header>
      <section className="overflow-x-auto rounded-2xl border border-neutral-200 bg-white">
          ...
          <tbody>
            {audit.data?.data.map((row) => (
              <tr className="border-t border-neutral-100" key={row.id}>
                ...
                <td className="px-5 py-4 font-bold">{row.action}</td>
        ...
        {audit.data?.data.length === 0 ? (
          <p className="p-6 text-neutral-500">Chưa có bản ghi audit.</p>
        ) : null}
      </section>
    </div>
  );
