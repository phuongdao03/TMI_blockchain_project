...
export function OperationsRiskChart({
  ...
  const items: RiskItem[] = [
    { label: "Hồ sơ trễ hạn", value: overdueReviews, color: "var(--theme-accent)" },
    { label: "Thanh toán lỗi", value: paymentFailures, color: "var(--theme-gold)" },
    { label: "Phát hành lỗi", value: blockchainFailures, color: "var(--theme-warning)" },
  ];
    ...
    >
      <div className="relative mx-auto size-40">
        <svg aria-hidden="true" className="size-full" viewBox="0 0 120 120">
          <circle cx="60" cy="60" fill="none" r={radius} stroke="var(--theme-elevated)" strokeWidth="14" />
          {total > 0
            ? items.map((item) => {
      ...
      <ul className="space-y-3" role="list">
        {items.map((item) => (
          <li className="flex items-center gap-3" key={item.label}>
            ...
            <span className="min-w-0 flex-1 text-sm text-neutral-600">{item.label}</span>
            <strong className="font-mono text-sm tabular-nums">{item.value}</strong>
          </li>
        ))}
      </ul>
    </div>
...
export function ReviewerWorkloadChart({
  ...
  return (
    <div aria-label="Biểu đồ khối lượng theo chuyên viên" className="space-y-4" role="img">
      {rows.length ? (
        rows.map((row) => (
          <div
          ...
          >
            <span className="truncate text-sm font-semibold" title={row.reviewerEmail}>
              {row.reviewerEmail}
            </span>
            <div className="order-3 col-span-2 h-3 overflow-hidden rounded-full bg-[var(--theme-elevated)] sm:order-none sm:col-span-1">
              <div
                ...
                style={{ width: `${Math.max(5, (row.activeAssignments / max) * 100)}%` }}
              />
            </div>
            <strong className="font-mono text-sm tabular-nums">{row.activeAssignments}</strong>
          </div>
        ))
      ) : (
