...
const ranking = {
  snapshot: {
    ...
    version: 2,
    formulaVersion: "effective-votes-v1",
    campaignRuleVersion: 1,
  ...
  items: [
    {
      ...
      slug: "heritage-work",
      title: "Heritage work",
      shortDescription: "A public ranked work.",
      ...
      displayOrder: 1,
      score: 8,
      effectiveVoteCount: 8,
    },
    {
      workId: "work-2",
      slug: "craft-work",
      title: "Craft work",
      shortDescription: "Another public ranked work.",
...
function renderBoard() {
  ...
  return render(
    <QueryClientProvider client={client}>
      <RankingBoard slug="heritage-campaign" />
    </QueryClientProvider>,
  );
}
...
describe("RankingBoard", () => {
  it("renders immutable snapshot metadata and ranked public works", async () => {
    ...
    renderBoard();
    expect(
      await screen.findByRole("heading", { name: "Bảng xếp hạng" }),
    ).toBeTruthy();
  ...
  it("shows a safe preparation state when the public snapshot is unavailable", async () => {
    ...
    expect(
      await screen.findByRole("heading", {
        name: "Kết quả đang được chuẩn bị",
      }),
    ).toBeTruthy();
  });
});
