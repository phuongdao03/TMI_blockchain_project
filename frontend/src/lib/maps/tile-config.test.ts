...
describe("resolveMapTileConfig", () => {
  it("does not use volunteer OSM tiles as an implicit production provider", () => {
    ...
    expect(resolveMapTileConfig("https://tiles.example/{z}/{x}/{y}.png", undefined, true)).toBeNull();
  });
  ...
  it("rejects insecure or malformed tile templates", () => {
    expect(resolveMapTileConfig("http://tiles.example/{z}/{x}/{y}.png", "Example Maps", true)).toBeNull();
    expect(resolveMapTileConfig("https://tiles.example/{z}/{x}.png", "Example Maps", true)).toBeNull();
  });

  it("keeps the default volunteer tiles available for local development", () => {
    expect(resolveMapTileConfig(undefined, undefined, false)?.url).toContain("tile.openstreetmap.org");
  });
});
